...
> `backups-recovery.sh` использовать с осторожностью (переносит архивные файлы в основную дирекорию `/`) - Если вы в первом аргументе($1) команды `backups-create.sh` передавали свои параметры отличные от тех, которые есть по-умолчанию, то при восстановлении, могут возникнуть проблемы.
